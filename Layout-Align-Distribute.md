...
Align
Users also have the ability to distribute the elements along an axis through selecting a group of elements and then using these choices to align with the element that is the furthest in the chosen option. The elements will then align the furthest side of the of the furthest element of the chosen side. Also all elements changing position are categorized as individual actions and the undo and redo buttons only change the position of one element at a time.
...
Distribute Vertical: Creates standard spacing on a vertical plane with selected elements in a given area between the elements on top and bottom of the selected groupin
